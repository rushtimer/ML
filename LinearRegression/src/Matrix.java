...
public class Matrix
{
	...
	public double[] rownum(int index) { return m_data.get(index); }
	...
	public void copyBlock(int destRow, int destCol, Matrix that, int rowBegin, int colBegin, int rowCount, int colCount)
	{
		...
		System.out.println("this cols"+ this.cols());
		*/
	...
	public Matrix scaleMatrix(double scalar, Matrix that)
	{
		Matrix matrix = new Matrix(that.rows(),that.cols());
		for(int i = 0; i<that.rows();i++)
		{
			for(int j = 0; j < that.cols();j++)
			{
				matrix.setMatrix(i, j, that.getvalue(i, j)*scalar);;
			}
		}
		return matrix;
	}
...
/*
	...
	{
		return m_data.get(rows)[cols];
	}
	// fill values to matrix
	public  void setMatrix(int rows, int cols, double val)
	{
		m_data.get(rows)[cols] = val;
	}
	public double fillValue(int rows, int cols, double val)
	{
		return m_data.get(rows)[cols] = val;
	}
	// print matrix
	public void print()
	{
    ...
    public Matrix plus(Matrix B) {
        Matrix A = this;
        if (B.rows() != A.rows() || B.cols() != A.cols()) throw new RuntimeException("Error: matrix dimensions don't match.");
        Matrix C = new Matrix(rows(), cols());
        for (int i = 0; i < rows(); i++)
            for (int j = 0; j < cols(); j++)
                C.m_data.get(i)[j] = A.m_data.get(i)[j] + B.m_data.get(i)[j];
        return C;
    }
 // Matrix subtract C = A - B
    ...
    public Matrix times(Matrix B) {
        Matrix A = this;
        if (A.cols() != B.rows()) throw new RuntimeException("Error: matrix dimensions don't match.");
        Matrix C = new Matrix(A.rows(), B.cols());
        for (int i = 0; i < C.rows(); i++)
            for (int j = 0; j < C.cols(); j++)
                for (int k = 0; k < A.cols(); k++)
                    C.m_data.get(i)[j] += (A.m_data.get(i)[k] * B.m_data.get(k)[j]);
        return C;
    }
    // Matrix a times Vector b
    public double matrixTimeVector(Matrix a, Vec b)
    {
    	double c = 0;
    	if (a.cols() != b.size())
    ...
    // compute a Matrix Xi- a Vector X^
    public Matrix matrixMinusVec(Matrix X, Vec x_cen)
    {
    	...
    	for(int i = 0; i< X.rows(); i++)
    	{
    		...
    		{
    			result.fillValue(i, j, X.getvalue(i, j)-x_cen.get(j));
    		}
    	}
    	//System.out.println("The result is: \n" + result);
    	return result;
    }
    // fill matrix with random number between -2 and 2
    public Matrix fillMatrix(Matrix that)
    {
		...
		for(int i = 0; i < that.rows();i++)
		{
			for(int j = 0; j < that.cols();j++)
			{
				that.fillValue(i, j, rand.nextGaussian());
			}
		}
    	return that;
    }
    public Matrix concatenateMatrix (Matrix a, Matrix b)
    {
    	if(a.cols() != b.cols())
    		throw new RuntimeException("Error: matrix dimensions don't match.");
    	a.newRows(b.rows());
    	...
    	for(int i = a.rows(); i< a.rows()+b.rows();i++)
    	{
    		for(int j = 0; j<a.cols();j++)
    		{
    			a.setMatrix(i, j, b.getvalue(i, j));
    		}
    	}
    	
    	return a;
    }
    public void MatrixToVector(Matrix a)
    {
    	Vec vector = new Vec(a.rows()*a.cols());
    	int count = 0;
    	for(int i =0; i < a.rows();i++)
    	{
    		for(int j = 0; j < a.cols();j++)
    		{
    			vector.set(count++, a.getvalue(i, j));
    		}
    	}
    }

    public Matrix arrayVecToMatrix(Vec[]array,Matrix source)
	{
		Matrix mat = new Matrix(source.rows(),source.cols());
		for(int i = 0; i < source.rows();i++)
		{
			for(int j = 0; j< source.cols();j++)
			{
				mat.setMatrix(i, j, array[i].get(j));
			}
		}
		//mat.print();
		return mat;
	...
	public Matrix outerProduct(Vec blame, Vec x) {
    	Matrix c = new Matrix(blame.len,x.len);
    	for(int i = 0; i< blame.len; i++)
    	{
    		for(int j = 0; j < x.len;j++)
    		{
    			c.setMatrix(i, j, blame.get(i)*x.get(j));
    		}
    	}
    	//c.print();
    	return c;
